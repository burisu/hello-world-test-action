...
async function exec() {
  try {
    ...
    const mergeMutation = await octokit.graphql(`
      mutation {
        mergeBranch(authorEmail: 'pgolfier.pro@gmail.com', base: '${mergeIn}', commitMessage: 'Merging ${pullRequest.headRefName} in ${mergeIn}', head: '${pullRequest.headRefName}', repositoryId: '${pullRequest.repository.databaseId}') {
          clientMutationId
          mergeCommit
        }
      }
